...
export async function download(context: ExtensionContext): Promise<void> {
  ...
  statusItem.text = `Downloading phpstan`;
  statusItem.show();

  const downloadUrl = 'https://github.com/phpstan/phpstan/releases/latest/download/phpstan.phar';
  ...
  resp.body.on('data', (chunk: Buffer) => {
    ...
    statusItem.text = `${p}% Downloading phpstan`;
  });

  const _path = path.join(context.storagePath, 'phpstan');
  const randomHex = randomBytes(5).toString('hex');
  const tempFile = path.join(context.storagePath, `phpstan-${randomHex}`);
